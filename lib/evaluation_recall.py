...
class BasicSceneGraphEvaluator:
    def __init__(self, mode, AG_object_classes, AG_all_predicates, AG_attention_predicates, AG_spatial_predicates, AG_contacting_predicates,
        ...
        self.total_rel_class_num = len(AG_all_predicates)

    def reset_result(self):
        self.result_dict[self.mode + '_recall'] = {10: [], 20: [], 50: [], 100: []}

    def print_stats(self, logger=None, mode='temporal'):

        assert mode in ('none', 'spatial', 'temporal', 'ensemble')

        if logger is None:
            print('======================{} ({})============================'.format(self.mode, mode))
            for k, v in self.result_dict[self.mode + '_recall'].items():
                recall, avg = np.mean(v), 0 
                meanRecall, avgRecall = 0, 0
                recall_per_class = []
                for idx in range(self.total_rel_class_num):
                     meanRecall += np.mean(self.result_dict[self.mode + '_meanRecall'][k][idx])
                     avgRecall += float(self.result_dict[self.mode + '_recall_hit'][k][idx+1]) / float(self.result_dict[self.mode + '_recall_count'][k][idx+1] + 1e-10)
                     
                     recall_per_class.append(np.mean(self.result_dict[self.mode + '_meanRecall'][k][idx]))

                print(f'mR@{k} ' + ' '.join([str(round(_, 4)) for _ in recall_per_class]))

                meanRecall, avgRecall = meanRecall / float(self.total_rel_class_num), avgRecall / float(self.total_rel_class_num)
                print(f'R@{k}: {recall:.4f} mR@{k}: {meanRecall:.4f} avgR@{k}: {avgRecall:.4f}')

        else:
            logger.info('======================{} ({})============================'.format(self.mode, mode))
            for k, v in self.result_dict[self.mode + '_recall'].items():
                recall, avg = np.mean(v), 0
                meanRecall, avgRecall = 0, 0
                recall_per_class = []
                ...
                logger.info(f'mR@{k} ' + ' '.join([str(round(_, 4)) for _ in recall_per_class]))

                meanRecall, avgRecall = meanRecall / float(self.total_rel_class_num), avgRecall / float(self.total_rel_class_num)
                logger.info(f'R@{k}: {recall:.4f} mR@{k}: {meanRecall:.4f} avgR@{k}: {avgRecall:.4f}')

    def evaluate_scene_graph(self, gt, pred, mode='temporal'):
        '''collect the groundtruth and prediction'''

        assert mode in ('none', 'spatial', 'temporal', 'ensemble')
        if mode == 'spatial':
            pred['attention_distribution'] = pred["spatial_attention_distribution"]
            pred['spatial_distribution'] = pred["spatial_spatial_distribution"]
            pred['contacting_distribution'] = pred["spatial_contacting_distribution"]
        elif mode == 'temporal':
            pred['attention_distribution'] = pred["temporal_attention_distribution"]
            pred['spatial_distribution'] = pred["temporal_spatial_distribution"]
            pred['contacting_distribution'] = pred["temporal_contacting_distribution"]
        elif mode == 'ensemble':
            pred['attention_distribution'] = pred["ensemble_attention_distribution"]
            pred['spatial_distribution'] = pred["ensemble_spatial_distribution"]
            pred['contacting_distribution'] = pred["ensemble_contacting_distribution"]
        ...
        for idx, frame_gt in enumerate(gt):
            ...
            evaluate_from_dict(gt_entry, pred_entry, self.mode, self.result_dict,
                               ...
                               total_rel_class_num=self.total_rel_class_num)

def evaluate_from_dict(gt_entry, pred_entry, mode, result_dict, method=None, threshold = 0.9, total_rel_class_num=26, **kwargs):
    """
    ...
    for k in result_dict[mode + '_recall']:
        ...
        if (mode + '_meanRecall') not in result_dict:
            result_dict[mode + '_meanRecall'] = {}
        if k not in result_dict[mode + '_meanRecall']:
            result_dict[mode + '_meanRecall'][k] = [[] for _ in range (total_rel_class_num)]

        hit, count = [0] * total_rel_class_num, [0] * total_rel_class_num
        ...
        for idx in range(len(match)):
            ...
            if k not in result_dict[mode + '_recall_hit']:
                result_dict[mode + '_recall_hit'][k] = [0] * (total_rel_class_num + 1)
            result_dict[mode + '_recall_hit'][k][int(local_label) + 1] += 1
            result_dict[mode + '_recall_hit'][k][0] += 1

            hit[int(local_label)] += 1
        ...
        for idx in range(gt_rels.shape[0]):
            ...
            if k not in result_dict[mode + '_recall_count']:
                result_dict[mode + '_recall_count'][k] = [0] * (total_rel_class_num + 1)
            result_dict[mode + '_recall_count'][k][int(local_label) + 1] += 1
            result_dict[mode + '_recall_count'][k][0] += 1

            count[int(local_label)] += 1

        for idx in range(total_rel_class_num):
            result_dict[mode + '_meanRecall'][k][idx].append(float(hit[idx]) / float(count[idx] + 1e-10))
