...
from lib.transformer import spatial_encoder, temporal_decoder, ensemble_decoder
...
class STKET(nn.Module):
    ...
    def __init__(self, mode='sgdet',
                 ...
                 N_layer_num=1, enc_layer_num=None, dec_layer_num=None,
                 pred_contact_threshold=0.5, window_size=4, trainPrior=None,
                 use_spatial_prior=False, use_temporal_prior=False):
        ...
        embed_vecs = obj_edge_vectors(obj_classes, wv_type='glove.6B', wv_dir='/data6/putao/STTran/data', wv_dim=200)
        ...
        if enc_layer_num > 0:
            self.spatial_encoder = spatial_encoder(enc_layer_num=enc_layer_num, embed_dim=1936, nhead=8, dim_feedforward=2048, dropout=0.1,
                trainPrior=trainPrior, use_spatial_prior=use_spatial_prior, obj_class_num=len(self.obj_classes), 
                attention_class_num=self.attention_class_num, spatial_class_num=self.spatial_class_num, contact_class_num=self.contact_class_num)

        if dec_layer_num > 0:
            self.temporal_decoder = temporal_decoder(dec_layer_num=dec_layer_num, embed_dim=1936, nhead=8, dim_feedforward=2048, dropout=0.1, 
                pred_contact_threshold=pred_contact_threshold,
                trainPrior=trainPrior, use_temporal_prior=use_temporal_prior, obj_class_num=len(self.obj_classes), 
                attention_class_num=self.attention_class_num, spatial_class_num=self.spatial_class_num, contact_class_num=self.contact_class_num)

        if (enc_layer_num > 0) and (dec_layer_num > 0):
            self.ensemble_decoder = ensemble_decoder(dec_layer_num=2, embed_dim=1936, nhead=8, dim_feedforward=2048, dropout=0.1, 
                pred_contact_threshold=pred_contact_threshold, window_size=window_size, obj_class_num=len(self.obj_classes), 
                attention_class_num=self.attention_class_num, spatial_class_num=self.spatial_class_num, contact_class_num=self.contact_class_num)
        ...
        print('[STTran] Mode: {}, Pred_contact_threshold: {:.1f}, window_size: {:d}'.format(self.mode, self.pred_contact_threshold, self.window_size))
    ...
    def forward(self, entry):
        ...
        x_semantic = torch.cat((subj_emb, obj_emb), 1)          # x_semantic: tensor, (pair_nums, 200 + 200)

        rel_features = torch.cat((x_visual, x_semantic), dim=1) # rel_features: tensor, (pair_nums, 512 + 512 + 512 + 200 + 200)
        ...
        entry["contacting_distribution"] = self.c_rel_compress(rel_features)
        ...
        for _ in range(self.N_layer_num):
            ...
            if self.enc_layer_num > 0:
                spatial_features, spatial_priors = self.spatial_encoder(features=rel_features, im_idx=entry['im_idx'], entry=entry, mode=self.mode)

                # Predict by spatial features
                entry["spatial_attention_distribution"] = self.spa_a_rel_compress(spatial_features)
                entry["spatial_spatial_distribution"] = self.spa_s_rel_compress(spatial_features)
                entry["spatial_contacting_distribution"] = self.spa_c_rel_compress(spatial_features)
                
                entry["spatial_spatial_distribution"] = torch.sigmoid(entry["spatial_spatial_distribution"])
                entry["spatial_contacting_distribution"] = torch.sigmoid(entry["spatial_contacting_distribution"])

                # Predict by spatial priors
                if self.use_spatial_prior:
                    entry["spatial_prior_attention_distribution"] = self.spa_a_rel_compress(spatial_priors)
                    entry["spatial_prior_spatial_distribution"] = self.spa_s_rel_compress(spatial_priors)
                    entry["spatial_prior_contacting_distribution"] = self.spa_c_rel_compress(spatial_priors)
                    
                    entry["spatial_prior_spatial_distribution"] = torch.sigmoid(entry["spatial_prior_spatial_distribution"])
                    entry["spatial_prior_contacting_distribution"] = torch.sigmoid(entry["spatial_prior_contacting_distribution"])

            contact_distribution = entry["spatial_contacting_distribution"] if self.enc_layer_num > 0 else entry["contacting_distribution"]

            # Temporal Decorder
            if self.dec_layer_num > 0:
                temporal_features, temporal_priors = self.temporal_decoder(features=spatial_features if self.enc_layer_num > 0 else rel_features, 
                    contact_distribution=contact_distribution, im_idx=entry['im_idx'], entry=entry, mode=self.mode)

                # Predict by temporal features
                entry["temporal_attention_distribution"] = self.tem_a_rel_compress(temporal_features)
                entry["temporal_spatial_distribution"] = self.tem_s_rel_compress(temporal_features)
                entry["temporal_contacting_distribution"] = self.tem_c_rel_compress(temporal_features)

                entry["temporal_spatial_distribution"] = torch.sigmoid(entry["temporal_spatial_distribution"])
                entry["temporal_contacting_distribution"] = torch.sigmoid(entry["temporal_contacting_distribution"])

                # Predict by temporal priors
                if self.use_temporal_prior:
                    entry["temporal_prior_attention_distribution"] = self.tem_a_rel_compress(temporal_priors)
                    entry["temporal_prior_spatial_distribution"] = self.tem_s_rel_compress(temporal_priors)
                    entry["temporal_prior_contacting_distribution"] = self.tem_c_rel_compress(temporal_priors)
                    
                    entry["temporal_prior_spatial_distribution"] = torch.sigmoid(entry["temporal_prior_spatial_distribution"])
                    entry["temporal_prior_contacting_distribution"] = torch.sigmoid(entry["temporal_prior_contacting_distribution"])
        ...
        if (self.enc_layer_num > 0) and (self.dec_layer_num > 0):        
            contact_distribution = entry["spatial_contacting_distribution"] if self.enc_layer_num > 0 else entry["contacting_distribution"]
            ensemble_features = self.ensemble_decoder(spatial_features=spatial_features, temporal_features=temporal_features, contact_distribution=contact_distribution, im_idx=entry['im_idx'], entry=entry, mode=self.mode)

            # ensemble
            entry["ensemble_attention_distribution"] = self.ens_a_rel_compress(ensemble_features)
            entry["ensemble_spatial_distribution"] = self.ens_s_rel_compress(ensemble_features)
            entry["ensemble_contacting_distribution"] = self.ens_c_rel_compress(ensemble_features)

            entry["ensemble_spatial_distribution"] = torch.sigmoid(entry["ensemble_spatial_distribution"])
            entry["ensemble_contacting_distribution"] = torch.sigmoid(entry["ensemble_contacting_distribution"])
